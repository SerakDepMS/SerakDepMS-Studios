document.addEventListener("DOMContentLoaded", () => {
    ...
    window.addEventListener("load", () => {
        ...
        if (loader) {
            // Pequeño respiro solo para que la transición se sienta suave,
            // ya no se fuerza una espera fija de 2 segundos.
            setTimeout(() => {
    ...
    projectCards.forEach((card) => {
        ...
        // Permite abrir el detalle del proyecto con teclado (Enter / Espacio)
    ...
    const animatedElements = document.querySelectorAll('.hero-content, .hero-image, .about-content, .about-image, .service-card, .project-card, .cta-wrapper, .manifesto-card, .timeline-item, .arch-card');
    
    animatedElements.forEach((el, index) => {
        el.style.opacity = 0;
        el.style.transform = "translateY(30px)";
        const delay = el.classList.contains('service-card') || el.classList.contains('project-card') ? (index % 3) * 0.1 : 0;
